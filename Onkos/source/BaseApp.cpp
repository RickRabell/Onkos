...
BaseApp::init() {
	...
	if (!m_spitfire.isNull()) {
		...
		if (!m_model || !m_model->load("Spitfire/spitfire.fbx")) {
			ERROR("Main", "InitDevice", "Failed to load SpitFire model.");
			return E_FAIL;
		}
		...
		if (FAILED(hr)) {
			ERROR("Main", "InitDevice",
				("Failed to initialize SpitFire Texture. HRESULT: " + std::to_string(hr)).c_str());
			return hr;
		}
	...
	if (!m_sciFiToad.isNull()) {
		m_toadModel = new Model3D("Frog/SciFiToad.fbx", ModelType::FBX);
		if (!m_toadModel || !m_toadModel->load("Frog/SciFiToad.fbx")) {
	...
	configurePbrMaterial(m_spitfirePbrMaterial);
	...
	m_spitfireMaterial.setMaterial(&m_spitfirePbrMaterial);
	m_spitfireMaterial.setAlbedo(&m_AlbedoSRV);
	m_spitfireMaterial.setNormal(&m_NormalSRV);
	m_spitfireMaterial.setMetallic(&m_MetallicSRV);
	m_spitfireMaterial.setRoughness(&m_RoughnessSRV);
	m_spitfireMaterial.setAO(&m_AOSRV);
	if (m_EmissiveSRV.m_textureFromImg) {
		m_spitfireMaterial.setEmissive(&m_EmissiveSRV);
	}
	...
	m_spitfireMaterial.getParams().emissiveStrength = 1.0f;
	...
	EU::TSharedPointer<MeshRendererComponent> meshRenderer = m_spitfire->getComponent<MeshRendererComponent>();
	if (!meshRenderer) {
		meshRenderer = EU::MakeShared<MeshRendererComponent>();
		m_spitfire->addComponent(meshRenderer);
	}
	meshRenderer->setMesh(&m_spitfireRenderMesh);
	meshRenderer->setMaterialInstance(&m_spitfireMaterial);
	...
	hr = m_postProcessSystem.init(m_device, 1280, 720);
	if (FAILED(hr)) {
		ERROR("Main", "InitDevice",
...
void BaseApp::handleEditorViewportResize()
{
	...
	m_postProcessSystem.resize(m_device,
		m_pendingViewportWidth,
		m_pendingViewportHeight);
