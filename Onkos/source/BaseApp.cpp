...
BaseApp::init() {
    ...
    */
    ...
    // -------------------------------------------------------------------------
    if (!m_spitFire.isNull()) {
      m_spitFireModel = new Model3D("Spitfire/spitfire.fbx", ModelType::FBX);
      if (!m_spitFireModel || !m_spitFireModel->load("SpitFire/spitfire.fbx")) {
        ERROR("Main", "InitDevice", "Failed to load Spitfire pistol model.");
        return E_FAIL;
      }

      hr = m_spitFireAlbedoSRV.init(m_device, "Spitfire/spitfire_d", PNG);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
             ("Failed to initialize Spitfire albedo texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
      }

      hr = m_spitFireNormalSRV.init(m_device, "Spitfire/spitfire_n", PNG);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
             ("Failed to initialize Spitfire normal texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
      }

      hr = m_spitFireMetallicSRV.init(m_device, "Spitfire/spitfire_m", PNG);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
             ("Failed to initialize Spitfire metallic texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
      }

      hr = m_spitFireRoughnessSRV.init(m_device, "Spitfire/spitfire_r", PNG);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
             ("Failed to initialize Spitfire roughness texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
      }

      hr = m_spitFireAOSRV.init(m_device, "Spitfire/spitfire_ao", PNG);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
             ("Failed to initialize Spitfire AO texture. HRESULT: " + std::to_string(hr)).c_str());
        return hr;
      }

      m_spitFire->setName("Spitfire");
      m_actors.push_back(m_spitFire);
      m_spitFire->getComponent<Transform>()->setTransform(EU::Vector3(-2.5f, -1.90f, 9.5f),
    ...
    m_spitFireMaterial.setMaterial(&m_pbrMaterial);
    m_spitFireMaterial.setAlbedo(&m_spitFireAlbedoSRV);
    m_spitFireMaterial.setNormal(&m_spitFireNormalSRV);
    m_spitFireMaterial.setMetallic(&m_spitFireMetallicSRV);
    m_spitFireMaterial.setRoughness(&m_spitFireRoughnessSRV);
    m_spitFireMaterial.setAO(&m_spitFireAOSRV);
    m_spitFireMaterial.getParams().baseColor = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
    m_spitFireMaterial.getParams().metallic = 1.0f;
    m_spitFireMaterial.getParams().roughness = 1.0f;
    m_spitFireMaterial.getParams().ao = 1.0f;
    m_spitFireMaterial.getParams().normalScale = 1.0f;
    m_spitFireMaterial.getParams().alphaCutoff = 0.5f;
    ...
    for (const MeshComponent& meshComponent : m_spitFireModel->GetMeshes()) {
      Submesh submesh{};
      hr = submesh.vertexBuffer.init(m_device, meshComponent, D3D11_BIND_VERTEX_BUFFER);
      if (FAILED(hr)) {
        ERROR("Main", "InitDevice",
      ...
      submesh.indexCount = meshComponent.m_numIndex;
      submesh.materialSlot = 0;
      m_spitFireRenderMesh.getSubmeshes().push_back(std::move(submesh));
    }
    ...
    EU::TSharedPointer<MeshRendererComponent> spitFireMeshRenderer = m_spitFire->getComponent<MeshRendererComponent>();
    if (!spitFireMeshRenderer) {
      spitFireMeshRenderer = EU::MakeShared<MeshRendererComponent>();
      m_spitFire->addComponent(spitFireMeshRenderer);
    }
    spitFireMeshRenderer->setMesh(&m_spitFireRenderMesh);
    spitFireMeshRenderer->setMaterialInstance(&m_spitFireMaterial);
...
BaseApp::destroy() {
  ...
  m_spitFireAlbedoSRV.destroy();
  m_spitFireNormalSRV.destroy();
  m_spitFireMetallicSRV.destroy();
  m_spitFireRoughnessSRV.destroy();
  m_spitFireAOSRV.destroy();
  ...
  delete m_model;
  m_model = nullptr;
  delete m_spitFireModel;
  m_spitFireModel = nullptr;
